...
module.exports.run = async (client, player, track) => {
  ...
  const Started = new EmbedBuilder()
    .setAuthor({
      name: `Now Playing`,
      iconURL:
        "https://cdn.discordapp.com/attachments/1185515072840273963/1195383828299128832/giphy.gif?ex=65b3caf1&is=65a155f1&hm=2621beee67f2f15cbb4ce7e43ee919b2f092953273156e93c98d8c568ba227ec&",
    })
    .setThumbnail(track.info.image)
    .setDescription(`**[${trackTitle}](${track.info.uri})**`)
    .addFields([
      {
        name: `<:author:1196080711082446959> Author:`,
        value: `${trackAuthor}`,
        inline: true,
      },
      {
        name: `<:requested:1196092196508680333> Requested By:`,
        value: `${track.info.requester}`,
        inline: true,
      },
      {
        name: `<:duration:1196535637294207097> Duration:`,
        value: `${trackDuration}`,
        inline: true,
      },
    ])
    ...
    .setFooter({
      text: `Loop Mode: ${capital(player.loop)} • Queue Left: ${
        player.queue.length
  ...
  const filters = new StringSelectMenuBuilder()
    ...
    .addOptions(
      new StringSelectMenuOptionBuilder()
        ...
        .setValue("clear")
        .setEmoji("<:clearfilters:1202690868981538826> "),
      new StringSelectMenuOptionBuilder()
        .setLabel("8D Filter")
        .setDescription("Click To Apply 8D Filter.")
        .setValue("8d")
        .setEmoji("<:filters:1202690364901691453>"),
      new StringSelectMenuOptionBuilder()
  ...
  collector.on("collect", async (message) => {
    if (message.customId === "loop") {
      if (!player) {
        ...
        await nplaying.edit({
          embeds: [Started],
          components: [menu, button, button2],
        });
      ...
      if (!player) {
        ...
        Started.setAuthor({
          name: `Song Paused`,
          iconURL:
            "https://cdn.discordapp.com/attachments/1185515072840273963/1196077071559049318/20240114_130254.gif?ex=65b65093&is=65a3db93&hm=7355d84c1d8434dd883ae00d753d8bb27be7ad53183e46af7bcf80b6845725ea&",
        });
      ...
      if (!player) {
        ...
        Started.setFooter({
          text: `Queue Left: ${player.queue.length} • Loop Mode: ${capital(
            player.loop,
          )} • Volume: ${player.volume}%`,
        });
      ...
      if (!player) {
        ...
        const playerDuration = player.currentTrack.info.isStream
          ? "LIVE"
          : trackDuration;
        const currentAuthor = player.currentTrack.info.author
          ? Author
          : "Unknown";
        const currentTitle = player.currentTrack.info.title
          ? Titles
          : "Unknown";
        ...
        if (player.currentTrack.info.sourceName === "youtube")
          sources = "Youtube";
        else if (player.currentTrack.info.sourceName === "soundcloud")
          sources = "SoundCloud";
        else if (player.currentTrack.info.sourceName === "spotify")
          sources = "Spotify";
        else if (player.currentTrack.info.sourceName === "applemusic")
          sources = "Apple Music";
        else if (player.currentTrack.info.sourceName === "bandcamp")
          sources = "Bandcamp";
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `**[${currentTitle}](${player.currentTrack.info.uri})**`,
          )
          .addFields([
      ...
      if (!player) {
        ...
        if (selectedFilter) {
          if (selectedFilter === "clear") {
            const embed = new EmbedBuilder()
              .setDescription(
                `<:successful:1201818375274369045> | Filters has been cleared.`,
              )
              .setColor(client.color);
            ...
            const embed = new EmbedBuilder()
              .setDescription(
                `<:successful:1201818375274369045> | 8D filter activated.`,
              )
              .setColor(client.color);
            ...
            const embed = new EmbedBuilder()
              .setDescription(
                `<:successful:1201818375274369045> | Earrape filter activated.`,
              )
              .setColor(client.color);
            ...
            const embed = new EmbedBuilder()
              .setDescription(
                `<:successful:1201818375274369045> | Nightcore filter activated.`,
              )
              .setColor(client.color);
            ...
            const embed = new EmbedBuilder()
              .setDescription(
                `<:successful:1201818375274369045> | Slowmode filter activated.`,
              )
              .setColor(client.color);
            ...
            const embed = new EmbedBuilder()
              .setDescription(
                `<:successful:1201818375274369045> | Vaporwave filter activated.`,
              )
              .setColor(client.color);
